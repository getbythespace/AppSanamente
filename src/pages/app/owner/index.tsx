...
const Bg = () => (
  <div className="pointer-events-none absolute inset-0 -z-10">
    <div className="absolute inset-0 bg-gradient-to-br from-[#FFF6EF] via-[#FFFBF7] to-[#F3F4F6] dark:from-[#0B0F14] dark:via-[#0D1218] dark:to-[#0F141A]" />
    <div className="absolute -top-24 -left-24 h-80 w-80 rounded-full bg-orange-300/25 blur-3xl dark:bg-orange-400/10" />
    <div className="absolute top-1/3 -right-24 h-80 w-80 rounded-full bg-amber-200/25 blur-3xl dark:bg-amber-300/10" />
  </div>
)

/* ---------- Pills/gradientes para roles (solo UI) ---------- */
function getRolePill(role: string) {
  ...
  return <span className={`px-2.5 py-1 rounded-full text-xs font-medium ring-1 ${map[role] || 'bg-slate-100 text-slate-700 ring-slate-200'}`}>{label[role] || role}</span>
}
...
function TopBar() {
  ...
  return (
    <header className="relative w-full">
      <div className="absolute inset-0 bg-gradient-to-r from-orange-600 to-orange-700 dark:from-orange-700 dark:to-orange-800" />
      <div className="relative max-w-7xl mx-auto h-16 px-6 flex items-center justify-between text-white">
        <div className="flex items-center gap-4">
          ...
          <div className="hidden sm:block px-3 py-1 bg-white/10 rounded-full text-sm font-medium">Dashboard Ejecutivo</div>
        </div>
        <div className="flex items-center gap-3">
          <ThemeToggle />
          <RoleSwitcher />
          <Link href="/app/profile" className="flex items-center gap-2 text-sm px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 transition-all">
            {icons.shield} Perfil
          </Link>
          <button onClick={handleSignOut} className="text-sm px-4 py-2 rounded-xl bg-red-600 hover:bg-red-700 border border-red-700 transition-all">Cerrar sesión</button>
        </div>
      </div>
...
/* ---------- Card glass (light/dark) ---------- */
function Card({ children, className = '', gradient = false }: { children: React.ReactNode; className?: string; gradient?: boolean }) {
  return (
    <section
      className={`rounded-2xl border ${gradient
        ? 'bg-gradient-to-br from-white/95 via-white/90 to-amber-50/60 dark:from-slate-900/60 dark:via-slate-900/50 dark:to-slate-900/40'
        : 'bg-white/95 dark:bg-slate-900/60'}
      border-slate-200 dark:border-slate-700 backdrop-blur-sm
      shadow-[0_1px_0_#fff_inset,0_10px_25px_rgba(234,88,12,0.06)]
      ...
      hover:ring-1 hover:ring-orange-200 dark:hover:ring-orange-300/30 hover:-translate-y-[1px] ${className}`}
    >
      {children}
    </section>
  )
...
/* ---------- Métrica con barra de progreso / dark ---------- */
function MetricCard({
  title, value, subtitle, icon, color = 'blue', trend, percentage
}: {
  title: string; value: string | number; subtitle?: string; icon: React.ReactNode
  color?: 'blue' | 'green' | 'purple' | 'orange' | 'red' | 'indigo' | 'teal' | 'yellow' | 'pink' | 'emerald'
  trend?: 'up' | 'down' | 'neutral'; percentage?: number
}) {
  const colorClasses = {
    blue: 'from-blue-500 to-blue-600', green: 'from-emerald-500 to-green-600', purple: 'from-purple-500 to-fuchsia-600',
    orange: 'from-orange-500 to-amber-600', red: 'from-rose-500 to-red-600', indigo: 'from-indigo-500 to-violet-600',
    teal: 'from-teal-500 to-teal-600', yellow: 'from-yellow-500 to-amber-600', pink: 'from-pink-500 to-fuchsia-600',
    emerald: 'from-emerald-500 to-emerald-600'
  } as const
  const ringColors: Record<string, string> = {
    blue: 'ring-blue-200', green: 'ring-emerald-200', purple: 'ring-purple-200', orange: 'ring-orange-200',
    red: 'ring-rose-200', indigo: 'ring-indigo-200', teal: 'ring-teal-200', yellow: 'ring-amber-200',
    pink: 'ring-pink-200', emerald: 'ring-emerald-200'
  }
  const trendIcons = {
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="mb-2 flex items-center gap-2">
            ...
            <span className={`inline-block h-2.5 w-2.5 rounded-full bg-gradient-to-br ${colorClasses[color]} ring-2 ${ringColors[color]} ring-offset-1 ring-offset-white dark:ring-offset-slate-900`} />
          </div>
          ...
          {percentage !== undefined && (
            <div className="mt-3">
              <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300 mb-1">
                <span>Progreso</span><span className="font-medium">{Math.min(percentage, 100)}%</span>
              </div>
              <div className="w-full h-2 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden">
                <div className={`h-2 bg-gradient-to-r ${colorClasses[color]} rounded-full transition-[width] duration-700`} style={{ width: `${Math.min(percentage, 100)}%` }} />
              </div>
            </div>
          )}
        </div>
        <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${colorClasses[color]} text-white flex items-center justify-center shadow-md`}>
          {icon}
        </div>
      </div>
    </Card>
...
export default withPageRole(function AdminUsersPage() {
  ...
  const openInvite = async () => {
    ...
    setForm({ firstName: '', lastNamePaternal: '', lastNameMaternal: '', rut: '', email: '', role: '', birthDate: '' })
    try {
            ...
            <div className="p-6 border-b bg-gradient-to-r from-emerald-50 to-white dark:from-slate-900/40 dark:to-slate-900/10 rounded-t-2xl border-slate-200 dark:border-slate-700">
              ...
              {planInfo && (
                <div className="mt-2 text-xs text-slate-600 dark:text-slate-300">
                  ...
                  {planInfo.plan === 'SOLO' && (<span className="ml-2">· máx. 1 asistente, sin psicólogos adicionales</span>)}
                </div>
              )}
            </div>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input className="w-full border rounded-lg px-3 py-2 shadow-sm dark:bg-slate-900/40 dark:border-slate-700 dark:text-slate-100"
                    value={form.lastNamePaternal} onChange={e => setForm(f => ({ ...f, lastNamePaternal: e.target.value }))}/>
                </div>
                <div>
                  <label className="text-xs text-slate-600 dark:text-slate-400">Apellido materno</label>
                  <input className="w-full border rounded-lg px-3 py-2 shadow-sm dark:bg-slate-900/40 dark:border-slate-700 dark:text-slate-100"
                    value={form.lastNameMaternal} onChange={e => setForm(f => ({ ...f, lastNameMaternal: e.target.value }))}/>
                </div>
                <div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {(['PSYCHOLOGIST','ASSISTANT','PATIENT'] as InviteRole[]).map(r => {
                    ...
                    return (
                      <button key={r} onClick={() => !disabled && setForm(f => ({ ...f, role: r }))} type="button"
                ...
                {!!adultError && <div className="mt-2 text-xs text-rose-600 dark:text-rose-300">{adultError}</div>}
                {planInfo?.plan === 'SOLO' && (
...
/* ------- Tile de stats pequeño (misma estética) ------- */
